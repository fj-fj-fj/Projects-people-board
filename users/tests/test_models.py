...
class UserTestCase(TestCase):
    ...
    def test_create_boss(self):
        ...
        self.assertEquals(field_label_user,'Имя пользователя')

        # subordinates one2one, projects m2m
        ...
        self.assertEquals(field_label_project_completed,'Успешно завершенные проекты')
        ...
        self.assertEquals(expected_object_name, str(self.boss))

        # email, get_absolute_url, is_active, is_boss
        self.assertEquals(self.boss.get_absolute_url(), '/boss/john-galt/')
        self.assertNotEquals(self.boss.get_absolute_url(), '/boss/John-Galt/')
    ...
    def test_create_employee(self):
        """Create and save a employee successfully"""
        User = get_user_model().objects.create_user(
        ...
        self.assertEquals(self.employee.get_position(), 'M')

        # __str__
        expected_object_name = self.employee.user.username
        self.assertEquals(expected_object_name, str(self.employee))

        # email, is_active, is_employee
    ...
    def test_create_project(self):
        ...
        self.assertEquals(field_heads, 'Руководители проекта')
        ...
        # title, description, get_absolute_url
        self.assertEquals(project.title, 'Agony soft')
        self.assertEquals(project.description, '')
        self.assertEquals(project.get_absolute_url(), '/project/agony-soft/')
        self.assertNotEquals(project.get_absolute_url(), '/project/Agony soft/')
